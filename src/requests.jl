...
function get_projects(token_tim,team)
    resp=HTTP.request("GET",SCINOTE_URL*"/api/v1/teams/$(team)/projects?page[size]=999",header(token_tim))
    body = String(resp.body)
...
"""
function get_experiments(token_tim,team,project,show_archived=false) 
    resp=HTTP.request("GET",SCINOTE_URL*"/api/v1/teams/$(team)/projects/$(project)/experiments?page[size]=999",header(token_tim))
    body = String(resp.body)
    data=JSON.parse(body)["data"] 
    n_experiments= length(data)
    if show_archived == false
        experiments = Dict(data[i]["attributes"]["name"] => data[i]["id"] for i in 1:n_experiments if data[i]["attributes"]["archived"] == false)
    else
...
function get_protocols(token_tim,team,project,experiment,task)
   resp=HTTP.request("GET",SCINOTE_URL*"/api/v1/teams/$(team)/projects/$(project)/experiments/$(experiment)/tasks/$(task)/protocols?page[size]=999",header(token_tim))
   body = String(resp.body)
   data=JSON.parse(body)["data"]
